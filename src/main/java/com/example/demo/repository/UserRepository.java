...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<Object[]> findAllPermissionsByUserNameAndUserRole(@Param("userName") String userName,
            @Param("role_id") int userRoleId);
    ...
    @Query(value = """
            SELECT p.permission_id,t.permission,p.can_create,p.can_read, p.can_update,p.can_delete
            FROM tbl_role_permission p, tbl_permissions t
            WHERE p.role_id = :roleId and p.permission_id = t.permission_id
            """, nativeQuery = true)
    List<Object[]> findAllPermissionsByRoleId(@Param("roleId") Integer roleId);
